...
N=2000; M=50;                         % # of symbols, oversampling factor
s1=pam(N,2,1); s2=pam(N,2,1);         % real 2-level signals of length N
mup1=zeros(1,N*M); mup1(1:M:end)=s1;  % zero pad T-spaced symbol sequence
mup2=zeros(1,N*M); mup2(1:M:end)=s2;  % to oversample by integer length M
...
m2=filter(p,1,mup2);                  % convolve pulse shape with data
