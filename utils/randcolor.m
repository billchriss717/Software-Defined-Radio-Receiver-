...
Ts=0.001; nyq=0.5/Ts;              % sampling interval and nyquist rate
ssf=(-N/2:N/2-1)/(Ts*N);           % frequency vector
x=randn(1,N);                      % N random numbers
fbe=[0 100 110 190 200 nyq]/nyq;   % definition of desired filter
damps=[0 0 1 1 0 0];               % desired amplitudes
fl=70;                             % filter size
b=firpm(fl,fbe,damps);             % design the impulse response
y=filter(b,1,x);                   % filter x with impulse response b
