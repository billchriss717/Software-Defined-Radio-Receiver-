...
[m,imax]=max(abs(fftrBPF(1:end/2)));  % find frequency of max peak
ssf=(0:length(rp))/(Ts*length(rp));   % frequency vector
freqS=ssf(imax)                       % freq at the peak
phasep=angle(fftrBPF(imax));          % phase at the peak
[IR,f]=freqz(h,1,length(rp),1/Ts);    % frequency response of filter
[mi,im]=min(abs(f-freqS));            % at freq where peak occurs
phaseBPF=angle(IR(im));               % angle of BPF at peak freq
