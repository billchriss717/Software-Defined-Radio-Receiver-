...
linelength=60;                        % approx # letters in each line
load OZ.mat                           % file for input
n=text(1:m); nline=n; nlet='x';       % initialize variables
for i=1:100                           % length of output in lines
  j=1;
  while j<linelength | nlet~=' '      % scan through file
    k=findstr(text,n);                % find all occurrences of seed
    ind=round((length(k)-1)*rand)+1;  % pick one
    nlet=text(k(ind)+m);
    if abs(nlet)==13                  % pretend carriage returns
      nlet=' ';                       % are spaces
    end
